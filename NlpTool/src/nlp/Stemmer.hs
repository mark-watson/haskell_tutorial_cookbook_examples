...
measure =
  length .
  filter not . init . (True :) . map head . group . byIndex (map . isConsonant)
...
cvc word
  ...
  | otherwise =
    ...
    isConsonant word (lastIndex - 2) && last word `notElem` "wxy"
  where
    lastIndex = length word - 1
...
statefulReplace predicate str end replacement
  ...
  where
    ...
    replaced
      | predicate part = Right (part ++ replacement)
      | otherwise = Left str
...
beforeStep1b word = fromMaybe (Left word) result
  where
    cond23 x = do
      v <- x
      either (const Nothing) (return . Right) v
    cond1 x = do
      v <- x
      return (Left v)
    result =
...
afterStep1b word = fromMaybe word result
  where
    double =
      endsWithDouble word && not (any ((`isSuffixOf` word) . return) "lsz")
    ...
    result =
      findStem (const True) word [("at", "ate"), ("bl", "ble"), ("iz", "ize")] `mplus`
      iif double (init word) `mplus`
      iif mEq1AndCvc (word ++ "e")

step1b = either id afterStep1b . beforeStep1b

step1c word = fromMaybe word result
  where
    result = replaceEnd containsVowel word "y" "i"
...
main
    ...
    --writeFile "out.txt" $ unlines $ map stem $ lines content
 = do
  let content =
        "The sport of hocky is about 100 years old by ahdi dates. American Football is a newer sport. Programming is fun. Congress passed a new budget that might help the economy. The President signed the tax bill."
   in print $ map stem $ words content
